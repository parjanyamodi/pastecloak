...
async function deriveKey(kyberSecret: Uint8Array, x25519Secret: Uint8Array): Promise<Uint8Array> {
  ...
  // Hash to derive final key (cast to avoid TS5.9 ArrayBuffer strictness)
  const hashBuffer = await crypto.subtle.digest('SHA-256', combined as Uint8Array<ArrayBuffer>);
